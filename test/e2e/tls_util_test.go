...
func verifyCASecret(t *testing.T, caSecret *v1.Secret, namespace string) {
	...
	caSecretFromCluster, err :=
		framework.Global.KubeClient.CoreV1().Secrets(namespace).Get(caConfigMapAndSecretName, metav1.GetOptions{})
	if err != nil {
		t.Fatal(err)
	}
...
func verifyAppSecret(t *testing.T, appSecret *v1.Secret, namespace string) {
	...
	appSecretFromCluster, err :=
		framework.Global.KubeClient.CoreV1().Secrets(namespace).Get(appSecretName, metav1.GetOptions{})
	if err != nil {
		t.Fatal(err)
	}

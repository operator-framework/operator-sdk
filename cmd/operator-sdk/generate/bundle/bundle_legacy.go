...
func (c *bundleCmd) setDefaultsLegacy() {
	if c.operatorName == "" {
...
func (c bundleCmd) runManifestsLegacy() (err error) {
	...
	defaultBundleDir := filepath.Join(c.manifestRoot, "bundle", c.operatorName)
	if c.inputDir == "" {
	...
	opts := []gencsv.LegacyOption{
		gencsv.WithBundleBase(c.inputDir, c.apisDir),
	...
	if !c.quiet {
		log.Info("Bundle manifests generated successfully")
	}

	return nil
}

// runMetadataLegacy generates a bundle.Dockerfile and metadata.
func (c bundleCmd) runMetadataLegacy() error {

	directory := c.inputDir
	if directory == "" {
		directory = filepath.Join("deploy", "bundle", c.operatorName, bundle.ManifestsDir)
	} else {

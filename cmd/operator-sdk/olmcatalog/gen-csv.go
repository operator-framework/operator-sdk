...
Configure CSV generation by writing a config file 'deploy/olm-catalog/csv-config.yaml`,
	...
	genCSVCmd.Flags().StringVar(&csvConfigPath, "csv-config", "",
		"Path to CSV config file. Defaults to deploy/olm-catalog/csv-config.yaml")
	genCSVCmd.Flags().BoolVar(&updateCRDs, "update-crds", false,
...
func verifyGenCSVFlags() error {
	...
	if fromVersion != "" && csvVersion == fromVersion {
		return fmt.Errorf(
			"from-version (%s) cannot equal csv-version; set only csv-version instead", fromVersion)
	}

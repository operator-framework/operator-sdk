	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if watch.ReconcilePeriod != expectedReconcilePeriod {
				t.Fatalf("Unexpected reconcilePeriod %v expected %v",
					watch.ReconcilePeriod, expectedReconcilePeriod)
			}
			...
			if watch.WatchDependentResources != watchDependentResourcesDefault {
				t.Fatalf("Unexpected watchDependentResources %v expected %v",
					watch.WatchDependentResources, watchDependentResourcesDefault)
			}
			...
			if watch.AnsibleVerbosity != ansibleVerbosityDefault {
				t.Fatalf("Unexpected ansibleVerbosity %v expected %v",
					watch.AnsibleVerbosity, ansibleVerbosityDefault)
			}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for idx, expectedWatch := range tc.expected {
				...
				if gotWatch.Role != expectedWatch.Role {
					t.Fatalf("The GVK: %v unexpected Role: %v expected Role: %v", gvk,
						gotWatch.Role, expectedWatch.Role)
				}
				if gotWatch.Playbook != expectedWatch.Playbook {
					t.Fatalf("The GVK: %v unexpected Playbook: %v expected Playbook: %v", gvk,
						gotWatch.Playbook, expectedWatch.Playbook)
				}
				...
				if gotWatch.Finalizer != expectedWatch.Finalizer {
					if gotWatch.Finalizer.Name != expectedWatch.Finalizer.Name ||
						gotWatch.Finalizer.Playbook != expectedWatch.Finalizer.Playbook ||
						gotWatch.Finalizer.Role != expectedWatch.Finalizer.Role ||
						reflect.DeepEqual(gotWatch.Finalizer.Vars["sentinel"],
				...
				if gotWatch.ReconcilePeriod != expectedWatch.ReconcilePeriod {
					t.Fatalf(
						"The GVK: %v unexpected reconcile period: %v expected reconcile period: %v", gvk,
						gotWatch.ReconcilePeriod, expectedWatch.ReconcilePeriod)
				}

				if expectedWatch.MaxWorkers == 0 {
					if gotWatch.MaxWorkers != tc.maxWorkers {
						t.Fatalf("Unexpected max workers: %v expected workers: %v",
							gotWatch.MaxWorkers, tc.maxWorkers)
					}
				} else {

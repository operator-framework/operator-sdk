...
// RunAnsibleOperator sets the default flags for base image and starts the ansible operator.
func RunAnsibleOperator() error {
	flags := setDefaultFlags()

	err := runAnsibleOperator(flags)
	if err != nil {
		return err
	}
	return nil
...
func setDefaultFlags() *aoflags.AnsibleOperatorFlags {
	aof := &flags.AnsibleOperatorFlags{}

	flag.BoolVar(&aof.InjectOwnerRef, "inject-owner-ref", true, "The ansible operator will "+
		"inject owner references unless this flag is false")
	flag.IntVar(&aof.MaxWorkers, "max-workers", 1, "Maximum number of workers to use. "+
		"Overridden by environment variable.")
	flag.IntVar(&aof.AnsibleVerbosity, "ansible-verbosity", 2, "Ansible verbosity. "+
		"Overridden by environment variable.")
	flag.StringVar(&aof.AnsibleRolesPath, "ansible-roles-path", "", "Ansible Roles Path. "+
		"If unset, roles are assumed to be in {{CWD}}/roles.")
	flag.StringVar(&aof.AnsibleCollectionsPath, "ansible-collections-path", "",
	...
	if err := setAnsibleEnvVars(aof); err != nil {
		log.Error(err, "Error setting env variables")
	}
...
func setAnsibleEnvVars(flags *aoflags.AnsibleOperatorFlags) error {
	if flags != nil {
		if len(flags.AnsibleRolesPath) > 0 {
			...
			log.Info("Set the value %v for environment variable %v.",
...
func runAnsibleOperator(flags *aoflags.AnsibleOperatorFlags) error {
	printVersion()

...
var log = logf.Log.WithName("helm-operator")
...
func setDefaultFlags() *hoflags.HelmOperatorFlags {
	hof := &hoflags.HelmOperatorFlags{}
	flag.IntVar(&hof.MaxWorkers, "max-workers", 1, "Maximum number of workers to use.")
	flag.DurationVar(&hof.ReconcilePeriod, "reconcile-period", time.Minute,
		"Default reconcile period for controllers")
	flag.StringVar(&hof.WatchesFile, "watches-file", "watches.yaml", "Path to the watches file to use")
	flag.Parse()

	return hof
}

// RunHelmOperator sets the default flags for base image and starts the ansible operator.
func RunHelmOperator() error {
	flags := setDefaultFlags()

	err := runHelmOperator(flags)
	if err != nil {
		return err
	}
	return nil
...
func runHelmOperator(flags *hoflags.HelmOperatorFlags) error {
	printVersion()

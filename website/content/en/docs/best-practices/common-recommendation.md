...
- Optimize the values of the manager resources in `config/manager/manager.yaml` according to the project requirements. It is recommend defined the resources limits in order to follow good practices and for security reasons. More info: [Managing Resources for Containers][k8s-manage-resources] and [Docker Security Cheat Sheet][docker-cheats].
- Look for `TODO(user)` into the source code generated by the CLI to ensure that you will able to do the suggested customizations.

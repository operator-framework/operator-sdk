...
deploy_operator() {
    ...
    kubectl create -f "$OPERATORDIR/deploy/crds/helm.example.com_nginxes.yaml"
...
remove_operator() {
    ...
    kubectl delete --wait=true --ignore-not-found=true -f "$OPERATORDIR/deploy/crds/helm.example.com_nginxes.yaml"

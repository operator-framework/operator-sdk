...
# Run tests
$ go test ./test/e2e/... -root=$(pwd) -kubeconfig=$HOME/.kube/config -globalMan deploy/crds/example.com_apps.yaml -namespacedMan deploy/namespace-init.yaml -v -parallel=2
```
...
```shell
$ kubectl delete -f deploy/crds/cache.example.com_memcacheds.yaml
```

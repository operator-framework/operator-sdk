...
DEFAULT_IMAGE = quay.io/operator-framework/helm-operator
IMAGE ?= $(DEFAULT_IMAGE)
...
all: format test build/helm-operator ## Test and Build the helm-operator

install: ## Install the helm-operator binary
	$(Q)$(GOARGS) go install $(GO_BUILD_ARGS) ./cmd/helm-operator
...
# Build and release helm-operator.
...
release: $(release_builds) $(release_builds:=.asc) ## Release helm-operator
...
build/%: $(SOURCES) ## Build the helm-operator binary
	$(Q)$(GOARGS) go build $(GO_BUILD_ARGS) -o $@ ./cmd/helm-operator

build/%.asc: ## Create release signatures for helm-operator release binaries
	$(Q){ \
...
test-sanity: tidy build/helm-operator lint
...
image-build: build/helm-operator-dev-linux-gnu image-scaffold
	./hack/image/build-helm-image.sh $(DEFAULT_IMAGE):dev

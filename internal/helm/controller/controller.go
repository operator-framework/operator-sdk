...
func Add(mgr manager.Manager, options WatchOptions) error {
	...
	if err := c.Watch(source.Kind(mgr.GetCache(), o), &libhandler.InstrumentedEnqueueRequestForObject{}, preds...); err != nil {
		return err
	}
		...
		for _, resource := range resources {
			...
			var setWatchOnResource = func(dependent runtime.Object) error {
				...
				if useOwnerRef { // Setup watch using owner references.
					err = c.Watch(source.Kind(mgr.GetCache(), unstructuredObj), crthandler.EnqueueRequestForOwner(mgr.GetScheme(),
						mgr.GetRESTMapper(), owner), predicate.DependentPredicate{})
					if err != nil {
						return err
					}

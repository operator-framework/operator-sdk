...
func (c *OLMCmd) AddToFlagSet(fs *pflag.FlagSet) {
	...
	fs.StringSliceVar(&c.IncludePaths, "include", nil,
		"Path to Kubernetes resource manifests, ex. Role, Subscription."+
			"These supplement or override defaults generated by up/down")
	fs.DurationVar(&c.Timeout, "timeout", defaultTimeout,
		"Time to wait for the command to complete before failing")
	fs.BoolVar(&c.ForceRegistry, "force-registry", false,
		"Force deletion of the in-cluster registry. This option is a no-op on 'up'.")
}

...
// validateConfig takes a viper config for a plugin and returns a nil error
// if valid or an error explaining why the config is invalid
func validateConfig(config pluginConfig, idx int, version string) error {
	...
	if config.External != nil {
		...
		if schelpers.IsV1alpha2(version) {
			return fmt.Errorf(
				"revert to v1alpha1 to use external plugins: external plugins are not currently supported with v1alpha2")
		}
	}
	if pluginType == "" {

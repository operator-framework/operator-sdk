...
// AnnotationsContainExamplesTest is a scorecard test that verifies that the CSV contains examples
// via the alm-examples annotation
...
func (t *AnnotationsContainExamplesTest) Run(ctx context.Context) *schelpers.TestResult {
	...
	if res.EarnedPoints == 0 {
		res.Suggestions = append(
			res.Suggestions, fmt.Sprintf("Add an alm-examples annotation to your CSV to pass the %s test",
				t.GetName()))
	}
	return res
}

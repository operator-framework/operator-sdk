...
// cache contains all generated files from calls to CachedGenerator.Run().
...
// CachedGenerator contains a set of pre-defined markers for generators and
// output rules from controller-tools used to configure a generator runtime.
// CachedGenerator caches each file generated in a global cache, to both
...
// NewCachedGenerator returns a CachedGenerator with a set of default
// generators and output rules. The returned CachedGenerator is lazily
// initialized.
...
func (g *CachedGenerator) AddOutputRule(defName string, rule genall.OutputRule) {
...
func (g *CachedGenerator) Run(rawOpts []string) error {
	g.init()

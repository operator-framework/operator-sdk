...
- Added group name validation in order to ensure that they will match with `"^[a-z-]+$"`. ([#1822](https://github.com/operator-framework/operator-sdk/pull/1822)). 
...
- **Breaking change:** Fix the error `unable to recognize "<CR>": no matches for kind "<CRD>" in version "<API>"` which is shown when the CR generated by the tool is applied in the cluster.([#1822](https://github.com/operator-framework/operator-sdk/pull/1822)). Following a few options to solve it.
    
    **IMPORTANT:** It is a breaking change which will just affects projects which are using an hyphen(`-`) in the group names. (`E.g -api-version=operator-name-group.example.com/v1alpha1`)  
    
    - **By operator-sdk:** Use the operator-sdk tool to recreate all CR/CRD's and API's then replace their implementations with the old ones. 
    - **By manual edition:** Rename all files and paths which are written with the group without the hyphen(`-`) for the original group name. See then in the `/<operator>/pkg/apis/` and `/<operator>/pkg/crds/` and fix all import paths in the files which are looking for the them without a hyphen.
